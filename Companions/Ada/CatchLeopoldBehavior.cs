...
namespace terraguardians.Companions.Ada
{
    public class CatchLeopoldBehavior : BehaviorBase
    {
        ...
        void OnAskToPlaceLeopoldOnTheGround()
        {
            if (Leopold != null)
            {
                if (Leopold.IsRunningBehavior && Leopold.GetGoverningBehavior() is not terraguardians.Companions.Leopold.HeldByAdaBehavior)
                {
        ...
        public override void Update(Companion companion)
        {
            ...
            else
            {
                ...
                {
                    Leopold.RunBehavior(new Leopold.HeldByAdaBehavior((TerraGuardian)companion));
                    switch(Main.rand.Next(3))
                    {
                        default:
                            Leopold.SaySomething("*Ack! No! Ada!*");
                            break;
                        case 1:

...
{
    public class AdaBase : TerraGuardianBase
    {
        ...
        public override BehaviorBase PreRecruitmentBehavior => new Companions.Ada.AdaRecruitmentBehavior();
        protected override CompanionDialogueContainer GetDialogueContainer => new AdaDialogues();
        public override CompanionData CreateCompanionData => new AdaData();
        ...
        public override void ModifyAnimation(Companion companion)
        {
            ...
            bool HasBunny = !companion.Crouching && companion.BodyFrameID != 33 && companion.BodyFrameID != 34 && companion.BodyFrameID != 37 && (companion.Data as AdaData).HasBunny;
            if(HasBunny)
            {
        ...
        public static void ApplyHeldBunnyAnimation(TerraGuardian Ada, bool HoldingLeopold = false)
        {
            short FrameID;
            ...
            switch(Ada.BodyFrameID)
            {
                default:
                    ...
                    UpdateBodyAnimation = Ada.BodyFrameID == 0 || Ada.BodyFrameID == 38;
                    break;
                case 20:
                    FrameID = 23;
                    break;
            ...
            {
                Ada.BodyFrameID = FrameID;
            }
            for(int i = 0; i < Ada.ArmFramesID.Length; i++)
            {
                if (Ada.HeldItems[i].ItemAnimation <= 0 && (i != 0 || Ada.GetCharacterMountedOnMe == null))
                    Ada.ArmFramesID[i] = FrameID;
            }
        }
        ...
        public override void CompanionDrawLayerSetup(bool IsDrawingFrontLayer, PlayerDrawSet drawSet, ref TgDrawInfoHolder Holder, ref List<DrawData> DrawDatas)
        {
            ...
            {
                bool HasBunny = !tg.Crouching && tg.BodyFrameID != 33 && tg.BodyFrameID != 34 && tg.BodyFrameID != 37 && tg.BodyFrameID != 32 && (tg.Data as AdaData).HasBunny;
                if (HasBunny)
                {
        ...
        public override void UpdateBehavior(Companion companion)
        {
            ...
            (companion.Data as AdaData).UpdateAdaData(companion);
            if (companion.Owner == null && !companion.TargettingSomething && !companion.IsRunningBehavior && Main.rand.Next(15) == 0 && !(companion.Data as AdaData).HasBunny && (companion.Data as AdaData).CanPickupLeopold)
            {
                foreach(Companion c in MainMod.GetActiveCompanions)
                {
                    if (c.IsSameID(CompanionDB.Leopold))
                    {
                        ...
                        {
                            companion.RunBehavior(new Ada.CatchLeopoldBehavior((TerraGuardian)c));
                            (companion.Data as AdaData).SetLeopoldPickupCooldown();
                        }
                        return;
                    }
                }
        ...
        {
            Outfits.Add(1, new Ada.RedHoodOutfit(true));
            Outfits.Add(2, new Ada.RedHoodOutfit(false));
        }
    }
}